...
AI/LLM spend waste auditor. Finds idle API keys, underused models, and wasted compute.
...
```
go install github.com/ppiankov/aispectre/cmd/aispectre@latest
```
...
Scans AI/LLM platforms for security findings.
...
- Does not remediate or modify AI/LLM platforms — scan is read-only
- Does not store findings or manage a findings database
- Does not replace dedicated AI/LLM platforms monitoring — point-in-time security audit only
...
```bash
aispectre scan --output json | jq '.summary'
aispectre scan --output json | jq '.findings[] | select(.severity == "critical")'
```
